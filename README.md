...
4. 출력 CSV 파일 경로를 선택하고 **녹화 시작** 버튼을 누르면 GUI가 BlueMuse를 통해 Bluetooth 연결을 확립한 뒤 BrainFlow 스트리밍을
   시작합니다. 녹화가 끝나면 자동으로 BlueMuse 연결을 해제하고, CSV 파일이 저장된 뒤 요약 정보가 창 하단 로그에 표시됩니다.
